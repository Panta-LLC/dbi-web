...
export function SponsorSection({
  ...
  const visibleItems = needsCarousel
    ? list.slice(index, index + VISIBLE_COUNT)
    : list;
      ...
      <div className="flex items-center justify-center gap-2 md:gap-4 px-4">
        ...
        <div className="flex-1 flex flex-row items-stretch justify-center gap-6 md:gap-10 min-w-0 max-w-5xl">
          {visibleItems.map((item, i) => (
            <div
              key={`${item.name}-${i}`}
              className="flex-1 min-w-0 flex flex-col items-center justify-center text-center px-4 py-6"
            >
              {item.logoSrc ? (
              ...
              <p className="mt-3 text-sm md:text-base font-bold text-slate-900">
                {item.name}
              </p>
              {item.tagline ? (
